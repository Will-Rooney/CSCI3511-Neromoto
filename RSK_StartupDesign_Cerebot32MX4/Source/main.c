...
void UserInit(void)
{
    ...
    {
		char greetingMessage[] = "Welcome, the Cerebot MX4 is ready for commands!\n(0=Stop, 1=Forward, 2=Reverse, 3=Left, 4=Right)\n\n";
		putUSBUSART(greetingMessage,strlen(greetingMessage));
	}*/
}//end UserInit
...
{   
    ...
    if(USBUSARTIsTxTrfReady())
    {
        // USB_Out_Buffer: char array with length 64 - stores data received from pc
		numBytesRead = getsUSBUSART(USB_Out_Buffer,64);
		if(numBytesRead != 0)
		{
			if (USB_Out_Buffer[0] == '0') {
				putrsUSBUSART("Stop\n"); // Send string data "Stop" to pc over usb port
				OC2R	= dtcMtrStopped; // Left Motor
				OC2RS	= dtcMtrStopped;
				OC3R	= dtcMtrStopped; // Right Motor
				OC3RS	= dtcMtrStopped;
			} else if (USB_Out_Buffer[0] == '1') {
				putrsUSBUSART("Forward\n");

				// Configure left motor direction
				prtMtrLeftDirClr	= ( 1 << bnMtrLeftDir );	// forward
				// Configure right motor diretion
				...
				OC3R	= dtcMtrMedium;
				OC3RS	= dtcMtrMedium;
			} else if (USB_Out_Buffer[0] == '2') {
				putrsUSBUSART("Reverse\n");

				// Configure left motor direction
				prtMtrLeftDirClr	= ( 1 << bnMtrRightDir );	// Reverse
				// Configure right motor diretion
				prtMtrRightDirSet	= ( 1 << bnMtrLeftDir );	// Reverse

				OC2R	= dtcMtrMedium;
				OC2RS	= dtcMtrMedium;
				OC3R	= dtcMtrMedium;
				OC3RS	= dtcMtrMedium;
			} else if (USB_Out_Buffer[0] == '3') {
				putrsUSBUSART("Left\n");
